...
<div class="invoice-container">
    ...
    @else
        <div class="separator"><strong>--- FACTURE D'AVOIR ---</strong></div>
        <div class="separator"> <strong>Ref. de fact. orig.</strong> : <span class="value"> {{ $CODEMECEFfacOriginale }} </span></div>
    @endif
    <hr> 
        ...
        @case('FV')
                ...
                @else
                    <div class="items">
                        @foreach ($itemFacture as $item)
                            <div class="item">
                                <div class="description"> {{ $item['quantity']}}  x {{number_format( $item['price'] , 0, ',', '.')}}<br> {{ $item['name'] }} (E)</div>
                                <div class="amount">{{ number_format(($item['price'] * $item['quantity']) , 0, ',', '.')}} </div>
                            </div><br>
                        @endforeach
                    </div>
    ...
    <div style="text-align: center;"> {{ $codeMECeFDGI }} </div>
    <div class="vendor-info">        
        <div><span class="label">MECeF NIN:</span> <span class="value">{{ $nim }} </span> </div>
        <div> <span class="label"> MECeF Compteurs: </span> <span class="value"> {{ $counters }} </span></div>
        <div> <span class="label">MECeF Heure:</span>  <span class="value"> {{ $dateTime }} </span></div>
