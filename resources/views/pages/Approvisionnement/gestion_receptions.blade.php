                        ...
                        <form method="POST" action="{{ route('receptions.store') }}">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="RefNumBonReception" id="RefNumBonReception"
                                        class="form-control">
                                </div>
                            </div>
                                                    ...
                                                    <select name="lignes[0][idMagasin]" class="form-select" required>
                                                        ...
                                                        @foreach ($magasins as $magasin)
                                                            <option value="{{ $magasin->idMgs }}">
                                                                {{ $magasin->libelleMgs }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td>
                                                    <input type="number" name="lignes[0][qteLivre]"
                ...
                <div class="modal-body">
                    <form method="POST" action="{{ route('receptions.update', $reception->idReception) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="RefNumBonReception" class="form-control" value="{{ $reception->RefNumBonReception }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Produits</label>
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @foreach ($reception->lignesReceptions as $index => $ligne)
                                        <tr>
                                            <td>
                                                <select id="productSelect" name="lignes[{{ $index }}][idP]" class="form-select" required>                   
                                                    @foreach ($produits as $produit)
                                                        <option value="{{ $produit->idP }}" 
                                                            {{ isset($ligne) && $produit->idP == $ligne->idP ? 'selected' : '' }}>
                                                            {{ $produit->NomP }}
                                                        </option>
                                                    @endforeach
                                                </select>                                                              
                                            ...
                                            <td>
                                                <select name="lignes[{{ $index }}][idMagasin]" class="form-select" required>
                                                    @foreach ($magasins as $magasin)
                                                        <option value="{{ $magasin->idMgs }}" {{ $magasin->idMgs == $ligne->idMagasin ? 'selected' : '' }}>
                                                            {{ $magasin->libelleMgs }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <input type="number" name="lignes[{{ $index }}][qteLivre]" class="form-control" value="{{ $ligne->qteReception }}" required>
                                            </td>
                                            <td>
                                                <input type="number" name="lignes[{{ $index }}][qteRestant]" class="form-control" value="{{ $ligne->qteRestante }}" readonly>
                                            </td>
                                            <td>
...
@endforeach
    ...
    <script>
        ...
        function updateRow(selectElement) {
            ...
            const qteRestante = parseFloat(selectElement.selectedOptions[0].getAttribute('data-qteRestante')) || 0;
            const prix = parseFloat(selectElement.selectedOptions[0].getAttribute('data-prix')) || 0;

            row.querySelector('input[name$="[qteRestant]"]').value = qteRestante;
            row.querySelector('input[name$="[prixUn]"]').value = prix;
        }
        ...
        function updateRowIndices() {
            ...
            rows.forEach((row, index) => {
                row.querySelectorAll('input, select').forEach(input => {
                    const name = input.name.replace(/lignes\\[\\d+\\]/, `lignes[${index}]`);
