...
@section('content')
    ...
    <div class="container-xxl">
                            ...
                            <table class="table datatable" id="datatable_1">
                                ...
                                <tbody>
                                    @foreach ($allcommande as $allcmd)
                                        <tr>
                                            <td>{{ $allcmd->numCmd }}</td>
                                            <td>{{ $allcmd->fournisseur ? $allcmd->fournisseur->identiteF : 'Non défini' }}
                                            </td>
                                            ...
                                            <td>
                                                <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                                    data-bs-target="#ModifyBoardModal{{ $allcmd->idCmd }}">
                                                    Modifier</button>
                                                <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                                    data-bs-target="#deleteBoardModal{{ $allcmd->idCmd }}">
                                                    Supprimer</button>
                                            </td>
                                        </tr>

                                        <div class="modal fade" id="deleteBoardModal{{ $allcmd->idCmd }}" tabindex="-1"
                                            aria-labelledby="deleteBoardModal{{ $allcmd->idCmd }}" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <form method="POST"
                                                            action="{{ route('commande.destroy', $allcmd->idCmd) }}">
                                                            @csrf
                                                            @method('DELETE')
        ...
        @foreach ($allcommande as $allcmd)
            <div class="modal fade" id="ModifyBoardModal{{ $allcmd->idCmd }}" tabindex="-1"
                aria-labelledby="ModifyBoardModal{{ $allcmd->idCmd }}" aria-hidden="true">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="mb-2 col-md-4">
                                        ...
                                        <input type="datetime-local" name="dateRemise" class="form-control"
                                            id="exampleInputPassword1" value="{{ $allcmd->dateRemise }}">
                                    </div>
                                    <div class="col-md-4">
                                        <label for="exampleInputPassword1" class="form-label">Description Commande</label>
                                        <input type="text" class="form-control" name="descCmd"
                                            id="exampleInputPassword1" value="{{ $allcmd->descCmd }}">
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    <div class="col-md-12">
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <button type="button" class="btn btn-secondary my-2 mx-3"
                                                onclick="ajouterCmd()">Ajouter une commande</button>
                                        </div>
                                        <table class="table table-bordered">
                                            ...
                                            <tbody id="ligneProduits">
                                                @foreach ($allcmd->lignesCommande as $index => $ligne)
                                                    <tr>
                                                        <input type="hidden" name="lignes[{{ $index }}][idLCmd]"
                                                            value="{{ $ligne->idLCmd }}">
                                                        ...
                                                        <td>
                                                            ...
                                                            <select id="productSelect"
                                                                ...
                                                                @foreach ($allproduits as $allproduit)
                                                                    <option value="{{ $allproduit->idP }}"
                                                                        {{ $ligne->idP == $allproduit->idP ? 'selected' : '' }}>
                                                                        {{ $allproduit->NomP }}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="number" name="lignes[{{ $index }}][qte]"
                                                                class="form-control qte" value="{{ $ligne->qteCmd }}">
                                                        </td>
                                                        <td>
                                                            <input type="number"
                                                                name="lignes[{{ $index }}][montantht]"
                                                                class="form-control montantht"
                                                                value="{{ $ligne->prix }}">
                                                        </td>
                                                        <td>
                                                            <input type="number" name="lignes[{{ $index }}][tva]"
                                                                class="form-control tva" value="{{ $ligne->TVA }}">
                                                        </td>
                                                        <td>
                                                            <button type="button" class="btn btn-danger delete-ligne"
                                                                data-id="{{ $ligne->idLCmd }}">Supprimer</button>
                    ...
                    <form method="POST" action="{{ route('ajouterCmd.store') }}">
                        ...
                        <div class="modal-body">
                            <div class="row mb-3">
                                <div class="col-md-4 mb-2">
                                    ...
                                    <select id="" name="identitefr" class="form-select">
                                        <option value="">Sélectionner un fournisseur</option>
                                        @foreach ($allfournisseurs as $allfournisseur)
                                            <option value="{{ $allfournisseur->idF }}">{{ $allfournisseur->identiteF }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <label for="exampleInputPassword1" class="form-label">Description Commande</label>
                                    <input type="text" class="form-control" name="descCmd" id="exampleInputPassword1"
                                        placeholder="">
                                </div>
                            </div>
                            <div class="row mb-3">
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody id="ligneProduits">
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="number" name="lignes[0][tva]" class="form-control tva">
                                                </td>
                                                {{-- <td>
